...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // === Cookies consent ===
  const COOKIE_KEY = 'usa_studium_cookie_consent';
  ...
  const writeConsent = (level) => {
    const payload = { level, timestamp: Date.now(), version: 1 };
    try { localStorage.setItem(COOKIE_KEY, JSON.stringify(payload)); } catch {}
    document.dispatchEvent(new CustomEvent('cookieConsentChanged', { detail: payload }));
  };
  ...
  if (banner) {
    ...
    const necessaryBtn = document.getElementById('cookieNecessary');
    const rejectBtn = document.getElementById('cookieReject');

    acceptBtn?.addEventListener('click', () => { writeConsent('all'); hideBanner(); });
    necessaryBtn?.addEventListener('click', () => { writeConsent('necessary'); hideBanner(); });
    rejectBtn?.addEventListener('click', () => { writeConsent('rejected'); hideBanner(); });
  }

  // Re-open banner from footer link or legal page
